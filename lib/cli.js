...
General options:
  -m, --mode={web|mob}       Test mode - Web application (using Selenium) or
      ...
      --rf={html|pdf|xml|excel}  Reports file format. Default is html.
  ...
  -l, --line                 Start the test case from the specified line number. Applies only for single test case run.
      --resume               Attach the test to any existing Selenium or Appium session. Default is false.
      --modules              A list of modules to load and initialize.
      --dispose={true|false} Dispose each module (WebDriver, etc.) automatically. Default is true.
...
var startup = {
    ...
    autoDispose: argv.dispose ? argv.dispose === 'true' : true,
    lineOffset: oxutil.parseLineOffsetArgument(argv.l || argv.line),
    autoResume: argv.resume ? argv.resume === 'true' : false,
    modules: oxutil.parseModulesArgument(argv.modules),
    reopenBrowser: argv.reopen ? argv.reopen === 'true' : false,
...
if (fileExt === '.js') {
    tsReadyPromise = oxutil.generateTestSuiteFromJSFile(srcFilePath, startup.parameters.file, startup.parameters.mode);
...
function prepareAndStartTheTest(ts) {
    // assigned file name as the test name if nothing else was assigned
    if (!ts.name) {
        ts.name = fileNameNoExt;
    }
    // take reopenBrowser option's value from json if specified
    startup.reopenBrowser = ts.options && ts.options.reopenBrowser ? ts.options.reopenBrowser : false;
