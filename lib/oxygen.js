...
module.exports.Runner = function () {
    ...
    module.init = function (options) {
        ...
        options.mode = options.mode ? options.mode : 'web';
        options.scriptContentLineOffset = module.getScriptContentLineOffset;
        // add user-defined line offset
        options.scriptContentLineOffset += options.lineOffset || 0;

        _localTime = _options.reporter && _options.reporter.localTime;
    ...
    module.getScriptContentLineOffset = 4;
    ...
    function processResultsAndRunTheNextIteration(msg) {
        ...
        curTCIt._status = _.isEmpty(failedSteps) && !msg.error ? STATUS.PASSED : STATUS.FAILED;
        if (msg.error) {
        ...
        if (tcindex >= ts.testcases.length) {
            ...
            tcit = 1;
            tsit++;
            // get to the next line of parameters file as test suite's iteration advances
            if (ts.paramManager) {
                ts.paramManager.readNext();
            }
        }
        // if SCRIPT_ERROR error was received, then stop further test execution
        if (msg.error && msg.error.type && _.includes(FATAL_ERROR_TYPES, msg.error.type)) {
