...
function init(options) {
    ...
    try {
        for (var i = 0; i < moduleFiles.length; i++) {
            ...
            moduleName = result[1];
            // if specified, ignore modules that are not in user-defined module list
            if (options.modules) {
                if (!options.modules.some(function(elm) {
                    return elm === moduleName;
                })) {
                    continue;
                }
            }
...
function disposeModules() {
    if (!modules || (options && !options.autoDispose)) {
